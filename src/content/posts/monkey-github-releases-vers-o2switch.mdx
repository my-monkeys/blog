...
**Leçon** : noter ce genre de pré-requis dans le `CLAUDE.md` du projet, parce que dans 6 mois si l'IP change, je vais re-perdre 30 minutes à debugger.
...
**Leçon** : les hypothèses qui marchent sur ta machine (PIDs uniques, processus utilisateur) ne tiennent plus dans un container. À chaque fois que tu touches à du PID-checking, demande-toi "est-ce que je tourne en PID 1 quelque part ?".
...
**Leçon** : les outils Unix font ce que tu leur demandes, pas ce que tu *voulais*. `--strip-components=1` strippe **toujours** 1 composant, peu importe que ce composant soit un fichier ou un dossier.
...
**Leçon** : à chaque fois que tu écris une chaîne dans une commande shell, demande-toi *d'où vient cette chaîne*. Si elle vient d'un input externe (filename, env var, body HTTP), tu dois la valider ou l'échapper. C'est de l'injection shell, OWASP top 10 année après année.
